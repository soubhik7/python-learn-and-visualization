...
class TraceConfiguration {
    ...
    public constructor(pythonConfig: vscode.WorkspaceConfiguration) {
        ...
        const userConfiguredMaxExecutedLines = pythonConfig.get<number>('maxExecutedLines');

        if (typeof userConfiguredMaxExecutedLines === 'number' && isFinite(userConfiguredMaxExecutedLines) && userConfiguredMaxExecutedLines > 0) {
            this.maxExecutedLines = userConfiguredMaxExecutedLines;
        } else {
            this.maxExecutedLines = 1000000; // Default to 1,000,000 if not a valid number
        }
...
export class PythonVisualizerConfiguration {
    public static getForResource(resouce: vscode.Uri) {
        return new PythonVisualizerConfiguration(resouce);
    }
    ...
    private constructor(resource: vscode.Uri) {
        const pythonConfig = vscode.workspace.getConfiguration('pythonVisualizer', resource);
    ...
    public equals(otherConfig: PythonVisualizerConfiguration) {
...
export class PythonVisualizerConfigurationManager {
    private readonly _visualizerConfigurationsForWorkspaces = new Map<string, PythonVisualizerConfiguration>();

    public loadAndCacheConfiguration(resource: vscode.Uri): PythonVisualizerConfiguration {
        const config = PythonVisualizerConfiguration.getForResource(resource);
        this._visualizerConfigurationsForWorkspaces.set(this.getKey(resource), config);
        return config;
    }

    public getConfigCacheForResource(resource: vscode.Uri): PythonVisualizerConfiguration {
        let config = this._visualizerConfigurationsForWorkspaces.get(this.getKey(resource));
        if (!config) {
            config = PythonVisualizerConfiguration.getForResource(resource);
            this._visualizerConfigurationsForWorkspaces.set(this.getKey(resource), config);
        }
        return config;
    }
    ...
    public hasConfigurationChanged(resource: vscode.Uri): boolean {
        ...
        const currentConfig = this._visualizerConfigurationsForWorkspaces.get(key);
        const newConfig = PythonVisualizerConfiguration.getForResource(resource);
