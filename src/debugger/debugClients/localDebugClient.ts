...
export class LocalDebugClient extends BaseDebugClient<LaunchRequestArguments> {
    ...
    constructor(args: LaunchRequestArguments, private _launcherScriptProvider: IDebugLauncherScriptProvider, private _visualizerManager: PythonVisualizerManager, private _logger: Logger) {
        super(args);
    }
    ...
    protected handleProcessOutput(proc: ChildProcess, failedToLaunch: (error: Error | string | Buffer) => void) {
        proc.on('error', error => {
            ...
            this._visualizerManager.dispose();
        });
